...
def test_terminal_review_is_readable_and_omits_raw_observed_fact_content():
    ...
    assert "Pace plan 3 — Draft — not active" in review
    assert "Next session" in review
    assert "20 km" in review
    assert "Coach assessment" in review
    assert "Observed training history and continuity" in review
    assert "Knowledge support" in review
...
def test_today_view_shows_the_next_session_when_today_has_no_session():
    ...
    assert "No session is planned today." in today
...
def test_cli_exposes_readable_review_and_today_views(monkeypatch, capsys):
    ...
    assert "Coach assessment" in capsys.readouterr().out
