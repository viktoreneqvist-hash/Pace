...
def test_activity_upsert_updates_an_existing_provider_record():
    ...
    with session_scope() as session:
        stored_activity, created = upsert_activity(session, first_activity)
        stored_id = stored_activity.id
        updated_record, created_again = upsert_activity(session, updated_activity)
...
def test_daily_metric_upsert_updates_one_record_per_day():
    with session_scope() as session:
        ...
        daily_metric, created_again = upsert_daily_metric(
            session,
            DailyMetric(
                date=date(2026, 6, 14),
                hrv_value=60.0,
