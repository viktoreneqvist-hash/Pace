...
def render_dashboard_html(*, state: AthleteState, trends: TrainingResponseTrends, plan: TrainingPlanFact | None, activities, recovery_observations=None) -> str:
    ...
    return f"""<!doctype html><html lang=\"en\"><head><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\"><title>Pace dashboard</title><style>{_STYLE}</style></head><body><main>
<header><p class=\"eyebrow\">PACE · LOCAL DASHBOARD</p><h1>{escape(state.as_of_date.isoformat())}</h1><p>Current snapshot. Charts show locally stored facts, not a readiness score.</p></header>
<section class=\"cards\"><article><small>Next session</small><strong>{escape(next_session)}</strong></article><article><small>Feedback, 28 days</small><strong>{trends.recent.outcomes.feedback_records} / {trends.recent_required_feedback_records}</strong></article><article><small>HRV baseline</small><strong>{_coverage(state, 'hrv')}</strong></article><article><small>Latest Garmin sync</small><strong>{escape(_sync_label(state))}</strong></article></section>
<section><h2>Training · 28 days</h2>{_training_chart(days, activity_seconds, activity_count)}<p class=\"muted\">Y-axis: hours per day. X-axis: Stockholm date. Hover over a bar for raw daily values.</p></section>
<section class=\"grid\"><article><h2>HRV · 28 days</h2>{_line_chart(days, [recovery.get(day).hrv_value if day in recovery else None for day in days], 'ms')}</article><article><h2>Resting heart rate · 28 days</h2>{_line_chart(days, [recovery.get(day).resting_heart_rate if day in recovery else None for day in days], 'bpm')}</article><article><h2>Sleep · 28 days</h2>{_line_chart(days, [recovery.get(day).sleep_duration_hours if day in recovery else None for day in days], 'hours')}</article><article><h2>Feedback & RPE</h2>{_feedback_panel(trends)}</article></section>
<section class=\"grid\"><article><h2>Active context</h2>{_context_panel(state)}</article><article><h2>Data quality</h2>{_quality_panel(state, trends)}</article></section>
<footer>Created locally by Pace. This read-only dashboard never changes plans, feedback, or context.</footer></main><div id=\"tooltip\" role=\"status\"></div><script>{_TOOLTIP_SCRIPT}</script></body></html>"""
...
def render_dashboard_fragment(
    *,
    ...
    recovery = {item.date: item for item in observations}
    return f"""
<section class="report-metrics" aria-label="Dashboard overview">
  <article><span>Next session</span><b>{escape(_next_session(plan, state.as_of_date))}</b></article>
  <article><span>Feedback, 28 days</span><b>{trends.recent.outcomes.feedback_records} / {trends.recent_required_feedback_records}</b></article>
  <article><span>HRV baseline</span><b>{_coverage(state, "hrv")}</b></article>
  <article><span>Latest Garmin sync</span><b>{escape(_sync_label(state))}</b></article>
</section>
<section class="report-section report-training">
  <h2>Training · 28 days</h2>
  {_training_chart(days, activity_seconds, activity_count)}
  <p class="muted">Y-axis: hours per day. X-axis: Stockholm date. Hover over a value for raw daily data.</p>
</section>
<section class="report-grid four">
  <article><h2>HRV · 28 days</h2>{_line_chart(days, [recovery.get(day).hrv_value if day in recovery else None for day in days], "ms")}</article>
  <article><h2>Resting heart rate · 28 days</h2>{_line_chart(days, [recovery.get(day).resting_heart_rate if day in recovery else None for day in days], "bpm")}</article>
  <article><h2>Sleep · 28 days</h2>{_line_chart(days, [recovery.get(day).sleep_duration_hours if day in recovery else None for day in days], "hours")}</article>
  <article><h2>Feedback &amp; RPE</h2>{_feedback_panel(trends)}</article>
</section>
<section class="report-grid two">
  <article><h2>Active context</h2>{_context_panel(state)}</article>
  <article><h2>Data quality</h2>{_quality_panel(state, trends)}</article>
</section>"""
...
def _training_chart(days, activity_seconds, activity_count):
    ...
    for index, day in enumerate(days):
        ...
        for offset, hours, css in ((0.12, ride_hours[index], "ride-bar"), (0.52, run_hours[index], "run-bar")):
            ...
            tooltip = f"{day.isoformat()} · Cycling: {ride_hours[index]:.2f} h · Running: {run_hours[index]:.2f} h · Sessions: {activity_count[day]}"
    ...
    return f'<p class="legend"><span class="ride-key">■</span> Cycling <span class="run-key">■</span> Running</p><svg class="training-chart" viewBox="0 0 {width} {height}" role="img" aria-label="Training hours per day, separated into cycling and running">{grid}<line x1="{left}" y1="{bottom + chart_height}" x2="{width - 8}" y2="{bottom + chart_height}"/>{bars}</svg>'


def _line_chart(days, values, label):
    present = [value for value in values if value is not None]
    if not present:
        return '<p class="muted">No stored data in this period.</p>'
...
def _feedback_panel(trends):
    ...
    reasons = ', '.join(f'{escape(code)} {count}' for code, count in current.reason_counts) or 'none reported'
    rpe = '—' if current.reported_rpe_average is None else f'{current.reported_rpe_average:.1f}/10 ({current.reported_rpe_data_points} sessions)'
    return f'<p><b>Completed:</b> {current.outcomes.completed} · <b>Limited:</b> {current.outcomes.completed_limited} · <b>Skipped:</b> {current.outcomes.skipped}</p><p><b>RPE:</b> {rpe}</p><p><b>Reasons:</b> {reasons}</p><p class="muted">{" · ".join(trends.limitations)}</p>'


def _context_panel(state):
    if not state.relevant_context.events:
        return '<p class="muted">No relevant context recorded in the current 7-day window.</p>'
...
def _quality_panel(state, trends):
    metrics = ''.join(f'<li>{escape(item.metric)}: {item.baseline_data_points}/{item.expected_baseline_days} baseline days</li>' for item in state.data_quality.recovery)
    return f'<ul>{metrics}<li>Trend status: {escape(trends.status)}</li></ul>'
...
def _sync_label(state):
    ...
    return 'missing' if sync is None else sync.requested_end_date.isoformat() if sync.requested_end_date else sync.status
...
def _next_session(plan, as_of_date):
    ...
    return 'No detailed sessions remain' if session is None else f'{session.scheduled_date}: {session.purpose}'
