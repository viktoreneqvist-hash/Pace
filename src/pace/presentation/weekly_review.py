...
def render_weekly_review_html(*, end_date, answer: WeeklyReviewAnswer) -> str:
    def section(title, values):
        return f"<section><h2>{title}</h2><ul>{''.join(f'<li>{escape(value)}</li>' for value in values) or '<li>Inget angivet.</li>'}</ul></section>"
    return f'<!doctype html><html lang="sv"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Pace veckoreview</title><style>{_STYLE}</style><main><header><p>PACE · VECKOREVIEW</p><h1>Vecka som slutar {escape(end_date.isoformat())}</h1></header><section><h2>Sammanfattning</h2><p>{escape(answer.summary)}</p></section>{section("Observationer", answer.observations)}{section("Rekommendationer", answer.recommendations)}{section("Osäkerheter", answer.uncertainties)}<footer>Skapad genom ett explicit Pace-anrop. Rapporten ändrar inte planen.</footer></main></html>'
